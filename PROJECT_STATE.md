...
| **Phase status** | Phase 0 complete and verified. Milestone 1.1 (identity domain) written, **not yet compiled** |
...
| **Next milestone** | Milestone 1.2 — Spring Security 7 filter chain, JWT, refresh-token rotation, auth endpoints |

---

## 2. Completed

### Phase 1 (in progress) — Milestone 1.1: identity domain
...
| **Starter coordinates may have changed** | Spring Boot 4 modularised the codebase into smaller jars. Artifact names in `libs.versions.toml` are conventional but unverified against Boot 4.1. | Generate a throwaway project at `start.spring.io` (Gradle Kotlin, Java 25, Boot 4.1, same starters) and diff its build file. **That tool is ground truth; this repo is not.** |
